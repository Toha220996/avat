...
function udalenie_metaboksa_metok() {
	$id = 'tagsdiv-type_work'; // у каждого метабокса есть свой ID, который можно глянуть в исходном коде страницы
	$tip = 'portfolio'; // откуда будем удалять - в данном случае со страниц редактирования записей
...
function dobavlenie_metaboksa(){
	$id = 'tagsdiv-type_work'; // ID может быть любой, главное, чтобы отличался от уже существующих метабоксов
	$zagolovok = 'Виды работ:';
...
function kod_metaboksa($post) {  
	...
	$vse_metki = get_terms('type_work', array('hide_empty' => 0) ); 
 
	// а теперь - все метки, которые присвоены к записи
	$vse_metki_posta = get_the_terms( $post->ID, 'type_work' );  
